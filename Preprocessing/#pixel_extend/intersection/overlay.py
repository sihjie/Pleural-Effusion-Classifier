...
base_mask_folder = 'Preprocessing/pixel_extend/intersection'
image_folder = 'Preprocessing/Cropped'
predict_mask_folder = 'Segmentation/UNetPlusPlus/results/all_predicted/img'
out_folder = 'Preprocessing/pixel_extend/intersection/overlay_img'
os.mkdir(out_folder)
...
for image_filename in os.listdir(image_folder):
    ...
    for i, folder in enumerate(dilation_folders):
        ...
        mask_path = os.path.join(dilation_folder_path, image_filename.replace("de_crop.jpg", f"extended_{folder[-2:]}_interwindow.jpg"))
        # print(mask_path)
    ...
    # cv2.waitKey(0)
    cv2.imwrite(os.path.join(out_folder, f'{image_filename[:4]}_overlay.jpg'), results)
    
print("Complete!")

# image = cv2.imread('Preprocessing/Cropped/0027_de_crop.jpg')
# overlay_image = image.copy()
# mask = cv2.imread('Preprocessing/pixel_extend/intersection/roi_extend_15/0027_extended_15_interwindow.jpg')
# mask_gray = cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY)
# mask_binary = (mask_gray > 127).astype(np.uint8)
...
# alpha = 0.2
# temp = cv2.addWeighted(color_mask, alpha, overlay_image, 1 - alpha, 0, overlay_image)
# cv2.imwrite('Preprocessing/asdoifje.jpg', temp)