...
def get_roi(original_folder, extend_mask_folder, output_folder):
    for filename in os.listdir(original_folder):
        original_path = os.path.join(original_folder, filename)
        mask_path = os.path.join(extend_mask_folder, 
                                 filename.replace("de_crop.jpg", f"extended_{extend_mask_folder[-2:]}_interwindow.jpg"))
        ...
        output_path = os.path.join(output_folder, filename.replace("de_crop.jpg", f"roi_extend_{extend_mask_folder[-2:]}.jpg"))
...
for i, folder in enumerate(dilation_folders):
    folder_path = os.path.join(base_mask_folder, folder)
    print(folder_path)

    roi_output_folder = f'Preprocessing/pixel_extend/intersection/inter_roi/roi_extend_{folder[-2:]}'
