...
from .data_preparation import prepare_data, augment_data, PleuralEffusionDataset
from .model import PleuralEffusionClassifier
from .loss_function import ClassBalancedFocalLoss, F1Loss
...
class ConfigManager:
    ...
    def __init__(self, config_path=None, config=None):
        ...
        self.patience = self._config['model_parameter']['patience']
        ...
        self.FILE_PATH = 'Classification/DL/image_only'
        ...
        self.result_folder = os.path.join(
            self.FILE_PATH,
            'results',
            f'{self.image_type} --{self.model_name}',
        )
...
class Plotter:
    ...
    @staticmethod
    def plot_accuracy_curve(train_accuracies, val_accuracies, path):
...
class PleuralEffusionTrainer:
    ...
    def _get_loss_function(self):
        ...
        elif self.cfg.loss_function == 'cb-focal':
            cb_focal_loss = ClassBalancedFocalLoss(beta=0.9999, gamma=1.0)
            print("Using Class Balanced Focal Loss")
            return cb_focal_loss

        elif self.cfg.loss_function == 'f1':
            f1_loss = F1Loss()
            print("Using F1 Loss")
            return f1_loss
    ...
    def run_kfold_training(self):
        ...
        full_train_dataset, self.test_dataset = prepare_data(
        ...
        for train_indices, val_indices in kf.split(full_train_dataset, full_train_dataset.labels):
            ...
            self.logger.info(f"Fold {fold} - Train indices: {train_indices} | Val indices: {val_indices}\n\n")
            ...
            if self.cfg.aug:
                augmented_images, augmented_labels = augment_data(train_image_paths, train_labels)
                self.logger.info(
            ...
            mean, std = self._compute_mean_std(train_image_paths, train_labels)
            ...
            for epoch in range(self.cfg.epochs):
                ...
                if val_loss < best_loss:
                    ...
                    print(f"Early stopping: patience count reset to 0")

                else:
                    patience_count += 1
                    if patience_count >= self.cfg.patience:
                        print("Early stopping!!")
                        self.logger.info(f"Early stopping triggered at {epoch+1} epochs")
                        break
                print(f"Early stopping: {patience_count} / {self.cfg.patience}")
                self.logger.info(f"Early stopping: {patience_count} / {self.cfg.patience}")
            ...
            Plotter.plot_loss_curve(train_losses, val_losses, loss_curve_path)
            Plotter.plot_accuracy_curve(train_accuracies, val_accuracies, acc_curve_path)
        ...
        return final_results_df, mean_std_fold_df

    def _compute_mean_std(self, image_paths, labels):
...
class PleuralEffusionTester:
    def __init__(self, config_manager: ConfigManager, results_path: str, test_dataset: PleuralEffusionDataset):
        ...
        self.OUTPUT_CSV = os.path.join(self.cfg.result_folder, '5-fold_testing_inference.csv')
...
class PleuralEffusionFinalPredictor:
    def __init__(self, config_manager: ConfigManager, results_path: str, test_results_df: pd.DataFrame):
        ...
        self.OUTPUT_CSV = os.path.join(self.cfg.result_folder, '5-fold_predict_all.csv')
    ...
    def _predict_all(self):
        ...
        model.eval()
        with torch.no_grad():
            for images, labels, filenames in final_loader:
                images, labels = images.to(device), labels.to(device)
                outputs = model(images)
                _, predicted = torch.max(outputs.data, 1)
...
def run_all_combinations(config_path='Classification/DL/image_only/config.yaml'):
    timestamp = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
    combo_logger = setup_logger(
        logger_name="AllParameterCombinations", 
        log_file=f"Classification/DL/image_only/results/{timestamp}_all_parameter_combinations.log"
    )
    ...
    param_combinations = itertools.product(
        image_type_options, model_options, loss_options, lr_options, aug_options
    )

    for img_type, model_name, loss_func, lr, aug in param_combinations:
        print(f"{'='*60}\nRunning parameters: image_type = {img_type}, model = {model_name}, loss function = {loss_func}, lr = {lr}, aug = {aug}")
        ...
        final_results_df, mean_std_fold_df = trainer.run_kfold_training()
        ...
        combo_logger.info(f"""
            ...
            Augmentation = {aug}
        """)        
        combo_logger.info(f"5-fold Training Results: \n{final_results_df}\n")
        combo_logger.info(f"Mean & Std of all folds: \n{mean_std_fold_df}\n\n")
        # combo_logger.info(f"======Testing Results======\n")
...
def main(run_all=False):
    config_path = 'Classification/DL/image_only/config.yaml'  # 你的 config 路徑
