...
class DL_MLTraining:
    ...
    def run_kfold_training(self, num_folds=5):
        ...
        for tfold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(full_train_dataset)), full_train_dataset.labels)):
            ...
            logging.info(f"Fold {fold} - Train image paths: {train_image_paths} | Val image paths: {val_image_paths}")
            ...
            # logging.info(f"extractor path is: {extractor_pth}")
            # print(f"extractor path is: {extractor_pth}")
            #@@@@@@@@@
            ...
            # train_deep_features, train_clinical, _, pca = extractor_trainer.extract_features(model, train_loader, device, ncomponents=1)  # ncomponents=1 隨便設，是給pca降維的，這邊不會回傳pca
            # print(f"dim of train_deep_features: {train_deep_features.shape[1]}\t\tdim of clinical features: {train_clinical.shape[1]}")
            # variances = np.var(train_deep_features, axis=0)
            # print("Variance of each feature dimension:", variances)
            # print("Matrix rank:", np.linalg.matrix_rank(train_deep_features))
            # scaler_pca = StandardScaler()
            # scaler_pca.fit(train_deep_features)
            # scaled_train_deep_features = scaler_pca.transform(train_deep_features)
            # variances = np.var(scaled_train_deep_features, axis=0)
            # print("Variance of each scaled feature dimension:", variances)
            # pca.fit(scaled_train_deep_features)
            # print(f"PCA selected training set components: {pca.n_components_}")
            # print(f"Explained variance ratio: {pca.explained_variance_ratio_.sum():.4f}")

            # reduced_train_deep_features = pca.transform(scaled_train_deep_features)
            # print(f"after PCA, dim of reduced_train_deep_features features: {reduced_train_deep_features.shape}")
            #@@@@@@@@
            ...
            val_deep_features, val_clinical, _, _ = extractor_trainer.extract_features(model, val_loader, device, ncomponents=1)  # ncomponents=1 隨便設，是給pca降維的，這邊不會回傳pca
            print(f"dim of val_deep_features: {val_deep_features.shape[1]}\t\tdim of clinical features: {val_clinical.shape[1]}")
            scaled_val_deep_features = scaler_deep_feature.transform(val_deep_features)
            reduced_val_features = pca.transform(scaled_val_deep_features)
            logging.info(f"after PCA, dim of reduced_val_features features: {reduced_val_features.shape[1]}")
            print(f"PCA selected validation set components: {pca.n_components_}")

            scaled_val_clinical = scaler_clinical.transform(val_clinical)
            val_combined_features = np.concatenate([scaled_val_clinical, reduced_val_features], axis=1)
            ...
            model_metrics = ml.train_all_model(scaled_train_deep_feature, train_labels, scaled_val_deep_features, val_labels, model_metrics)
...
class TrainFeatureExtractor:
    ...
    def train_extractor(self, train_image_paths, train_clinical_features, train_labels, val_image_paths, val_clinical_features, val_labels):
        ...
        features_array, clinical_array, labels, pca = self.extract_features(model, full_loader, device)

        return features_array, clinical_array, labels, pca, self.pth_path
    ...
    def extract_features(self, model, dataloader, device, ncomponents=0.95):
        ...
        return features_array, clinical_array, labels_array, pca
