...
from Classification.DL_ML.plotter_todf import Plotter, ReportToDataFrame, MetricsToDatafram, SHAPPlotter

class DL_MLTraining:
    def __init__(self, config_manager: ConfigManager, extract_folder):
        self.cfg = config_manager
        self.extract_folder = extract_folder
        self.dimred_method = self.cfg.dimred_method
    ...
    def run_kfold_training(self, num_folds=5):
        ...
        for tfold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(full_train_dataset)), full_train_dataset.labels)):
            ...
            extractor_pth = os.path.join(self.extract_folder, f"checkpoint_fold_{fold}.pth")
            logging.info(f"extractor path: {extractor_pth}")

            model, transform_normalize, device = self._load_checkpoint(extractor_pth)

            # === Extract deep features from the training set ===
            ...
            logging.info(f"dim of train deep features: {train_deep_features.shape[1]}\t\tdim of train clinical features: {train_clinical.shape[1]}")
            ...
            # training set deep features scaling
            ...
            # determine dimension reduction method
            logging.info(f"Dimensionality reduction method: {self.dimred_method}")
            if self.dimred_method == "pca":
                dr = PCA(n_components=0.95, svd_solver='full')
            elif self.dimred_method == "kpca":
                dr = KernelPCA(
                    n_components=45,
                    kernel='rbf',
                    gamma=1.0 / scaled_train_deep_features.shape[1],
                    fit_inverse_transform=True
            )
            elif self.dimred_method == "tsne":  # t-SNE 通常直接對資料做 fit_transform，無法做 transform
                perp_train = min(30, scaled_train_deep_features.shape[0] - 1)
                dr = TSNE(
                    n_components=2,        # 嵌入到 2 維
                    perplexity=perp_train,         # 建議 5~50 之間
                    n_iter=1000,
                    random_state=42,
                    init='pca'             # 以 PCA 結果初始化可加速收斂
                )

            elif self.dimred_method == "umap":
                dr = umap.UMAP(
                    n_components=45,       
                    n_neighbors=15,        # 建議 5~50 之間
                    min_dist=0.1,
                    metric='euclidean',
                    random_state=42
                )

            if self.dimred_method in ["pca", "kpca", "umap"]:
                dr.fit(scaled_train_deep_features)  # fit dimension reduction method using scaled "training" set deep features
                joblib.dump(dr, os.path.join(self.cfg.RESULTS_FOLDER, f"{self.dimred_method}_fold_{fold}.pkl"))
                reduced_train_deep_features = dr.transform(scaled_train_deep_features)

                column_names = [f'RDF{i+1}' for i in range(reduced_train_deep_features.shape[1])]

            elif self.dimred_method == "tsne":
                reduced_train_deep_features = dr.fit_transform(scaled_train_deep_features)

            logging.info(f"after {self.dimred_method}, dim of reduced_train_deep_features features: {reduced_train_deep_features.shape}")
            ...
            # Visualization
            fig_folder = os.path.join(self.cfg.RESULTS_FOLDER, f"{self.dimred_method}_fig")
            os.makedirs(fig_folder, exist_ok=True)
            if self.dimred_method == "pca":
                Plotter.plot_pca(train_labels, reduced_train_deep_features, os.path.join(fig_folder, f"train_fold_{fold}_pca.png"))
            elif self.dimred_method == "kpca":
                Plotter.plot_kpca(train_labels, reduced_train_deep_features, os.path.join(fig_folder, f"train_fold_{fold}_kpca.png"))
            elif self.dimred_method in ["tsne", "umap"]:
                Plotter.plot_embedding(
                    self.dimred_method, 
                    train_labels, 
                    reduced_train_deep_features, 
                    os.path.join(fig_folder, f"train_fold_{fold}_{self.dimred_method}.png")
                )            

            scaler_clinical = StandardScaler()
            scaler_clinical.fit(train_clinical)

            # clinical features scaling
            ...
            logging.info(f"dim of train combined features: {train_combined_features.shape}")

            # # SMOTE
            # smote = SMOTE(random_state=42)
            # train_features, train_labels = smote.fit_resample(train_combined_features, train_labels)
            # logging.info(f"After SMOTE: {train_features.shape}")

            # # TomekLinks
            # tl = TomekLinks()
            # train_features, train_labels = tl.fit_resample(train_features, train_labels)
            # logging.info(f"After TomekLinks: {train_features.shape}\n\n")
              
            # === Extract deep features from the validation set ===
            ...
            logging.info(f"dim of val deep features: {val_deep_features.shape[1]}\t\tdim of val clinical features: {val_clinical.shape[1]}")
            ...
            if self.dimred_method in ["pca", "kpca", "umap"]:
                reduced_val_deep_features = dr.transform(scaled_val_deep_features)
            elif self.dimred_method == "tsne":  # t-SNE 不支援同一個 dr.transform，重新 fit_transform
                perp_train = min(30, scaled_val_deep_features.shape[0] - 1)
                reduced_val_deep_features = TSNE(
                    n_components=2, perplexity=perp_train, n_iter=1000,
                    random_state=42, init='pca'
                ).fit_transform(scaled_val_deep_features)

            logging.info(f"after PCA, dim of reduced_val_features features: {reduced_val_deep_features.shape[1]}")

            # Visualization
            if self.dimred_method == "pca":
                Plotter.plot_pca(val_labels, reduced_val_deep_features, os.path.join(fig_folder, f"val_fold_{fold}_pca.png"))
            elif self.dimred_method == "kpca":
                Plotter.plot_kpca(val_labels, reduced_val_deep_features, os.path.join(fig_folder, f"val_fold_{fold}_kpca.png"))
            elif self.dimred_method in ["tsne", "umap"]:
                Plotter.plot_embedding(
                    self.dimred_method, 
                    val_labels, 
                    reduced_val_deep_features, 
                    os.path.join(fig_folder, f"val_fold_{fold}_{self.dimred_method}.png")
                )  
            ...
            logging.info(f"dim of val combined features: {val_combined_features.shape}\n\n")
            ...
            # reducted deep features + clinical features
            model_metrics = ml.train_all_model(train_combined_features, train_labels, val_combined_features, val_labels, model_metrics)
            
            # applying SMOTE and TomekLinks to the reducted deep features
            # model_metrics = ml.train_all_model(train_features, train_labels, val_combined_features, val_labels, model_metrics)
            
            # just reducted deep features
...
def main():
    ...
    # NEED TO SET
    extract_folder = r'Classification\DL\image_only\results\expand_15 --efficientnetv2_s\20250506_215435_[f1]_0.001_aug_True' # MODIFY THIS

    # NEED TO SET config.yaml TOO, SETTINGS SAME AS THE EXTRACT ONE

    logging.info(f"Extract folder: {extract_folder}\n\n")
    trainer = DL_MLTraining(config_manager, extract_folder)
