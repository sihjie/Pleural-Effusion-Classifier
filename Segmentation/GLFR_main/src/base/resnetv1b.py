...
class ResNetV1b(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, dilated=True, deep_stem=False,
        ...
        if zero_init_residual:  # zero-initialize the last BN in each residual branch
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, norm_layer=nn.BatchNorm2d):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:        # if need downsample
    ...
    def extract_features(self, x):  # extract features of each layer for segmentation
        x = self.conv1(x)
        x = self.bn1(x)
