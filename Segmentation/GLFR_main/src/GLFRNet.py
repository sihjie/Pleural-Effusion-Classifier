...
class GFR(nn.Module):
    def __init__(self, channels, inter_channel, n_class, base=8, aux=False, coord=False):
        ...
        self.base = base * n_class  # d_k: the number of global descriptors
...
class LFR(nn.Module):
    def __init__(self,  channel_high, channel_low, out_channel, c_mid=64, scale=2, k_up=5, k_enc=3):
        ...
        self.conv_reduce = ConvBNReLU(channel_high, self.inter_channel, kernel_size=3, stride=1, padding=1, dilation=1) # Reduce the channel of the high-level feature map to inter_channel
        ...
        self.enc = ConvBNReLU(c_mid*2, k_up**2, kernel_size=k_enc, stride=1, padding=k_enc//2, dilation=1, use_relu=False)  # Generate weights in the upsampling process

        self.unfold = nn.Unfold(kernel_size=k_up, dilation=scale, padding=k_up//2*scale)

        self.conv_out = nn.Sequential(
            nn.Conv2d(self.inter_channel+channel_low , out_channel, kernel_size=3, padding=1, bias=False),  # c_in = higher_feature + lower_feature
    ...
    def forward(self, x_h, x_l):
        b, c, h, w = x_h.size() # batch_size, #channel, height, width
        h_, w_ = h * self.scale, w * self.scale # height, width after upsampling
        
        x1 = F.interpolate(self.conv1(x_h), size=(h_, w_), mode="bilinear", align_corners=True)   # higher feature upsample, x1.shpae: (b, c_mid, h_, w_)
        x2 = self.conv2(x_l)    # x2.shape: (b, c_mid, h_, w_)
        W = self.enc(torch.cat((x1, x2), dim=1))                                 # b * 100 * h_ * w_
...
class GLFRNet(SegBaseModel):
    def __init__(self, n_class, backbone='resnet34', aux=True, pretrained_base=True, mid_channel=320, c_mid = 256, base=8, **kwargs):
        ...
        channels.reverse()  # channels: [512, 256, 128, 64]
